...
// 	: offset(0), tranferData{}, leftString{}, _output(capacity), _capacity(capacity) {}
...
void StreamReassembler::push_substring(const string &data, const size_t index, const bool eof) {
    ...
    for (size_t i = offset; i < data.size(); ++i) {
    	size_t pos = (i + index) % _capacity;
        // make sure not to overwrite the previous data
        if (exist_data[pos] && unassembled_data[pos] != data[i])
            break;

        unassembled_data[pos] = data[i];
        if (!exist_data[pos]) {
            exist_data[pos] = true;
            ++unassembled_size;
        }
